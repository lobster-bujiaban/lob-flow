...
- [ ] 阶段 3：变量系统、控制流与可靠执行（部分完成）
- [ ] 阶段 4：知识库检索与 RAG 节点（部分完成）
- [ ] 阶段 5：工具、Agent 与人工介入（Tool 已完成，Agent 与人工介入未完成）
- [ ] 阶段 6：版本发布、API、权限与多租户（核心闭环已完成）
- [ ] 阶段 7：可观测、评测与生产化（基础观测与可靠性已完成）
- [ ] 阶段 8：Dify 源码映射与最终差异清单

未勾选不代表该阶段尚未开始，而是表示实施计划中的全部任务和验收标准还没有完成。
当前版本已经超过阶段 0～2 的首个里程碑，形成了可运行的平台 MVP：

- 工作流支持 Start、Template、LLM、Knowledge、Tool、Condition、Switch 和 Answer 节点。
- 支持结构化输入输出、变量引用、条件与多路分支、节点超时与重试、错误分支、取消和幂等调用。
- 支持知识库、文档分段、内置多语言 Embedding、关键词/向量/混合检索、Metadata Filter、检索节点及结果引用。
- 支持内置 Tool 插件和 Dify Plugin Daemon，提供插件安装、授权、启停、调用和审计闭环。
- 支持草稿、发布版本、回滚、调试运行、服务 API、API Key 和 Cron 定时触发。
- 支持平台账户、邀请、Workspace 成员及 Owner、Admin、Editor、Viewer 权限边界。
- 保存工作流、节点、模型和工具调用的运行事件、耗时、Token Usage 与错误信息。

距离完整目标仍缺少 Agent Loop、人工审批与恢复、并行/聚合/有限迭代、外部 Embedding 模型与专业向量数据库适配、
API 限流与配额、统一 Trace、成本分析、评测集、独立 Worker/队列、备份恢复验证，
以及 Dify 源码映射。当前自动化测试覆盖也不足，因此本项目应视为学习型平台 MVP，
不应直接宣称为完整的 Dify 替代品或未经验证的生产级平台。
...
设置姓名和密码。密码使用 scrypt 加盐哈希保存；登录会话随机生成、数据库仅保存哈希并在
7 天后过期，退出或停用账户后立即失效。`create-admin` 命令仅作为生产环境灾备入口。
...
以及 Start、Template、LLM、Knowledge、Tool、Condition、Switch、Answer 节点的添加和删除。
选中节点后可在右侧配置输入变量、Prompt、模型、知识库、工具参数、分支条件和结构化输出。
节点坐标与连线一同保存，提交时校验唯一入口、节点引用、分支端口、不可达节点和循环依赖；
运行时画布实时展示每个节点的状态、输出、错误分支和最终回答。
