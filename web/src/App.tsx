...
function MainApp({ signedOut }: { signedOut: () => void }) {
  ...
  return (
    <div className="dify-shell">
      ...
      {showAdmin && currentUser?.is_super_admin && <PlatformAdminModal currentUser={currentUser} workspaces={workspaces} close={() => setShowAdmin(false)} onError={showError} />}
...
function PlatformAdminModal({ currentUser, workspaces, close, onError }: { currentUser: User; workspaces: Workspace[]; close: () => void; onError: (reason: unknown) => void }) {
  const [users, setUsers] = useState<User[]>([]);
  const [workspaceMemberships, setWorkspaceMemberships] = useState<Record<string, WorkspaceMember[]>>({});
  const [inviteLink, setInviteLink] = useState("");
  const load = async () => {
    try {
      const [platformUsers, membershipResults] = await Promise.all([
        api.listPlatformUsers(),
        Promise.all(workspaces.map(async (workspace) => [workspace.id, await api.listMembers(workspace.id)] as const)),
      ]);
      setUsers(platformUsers);
      setWorkspaceMemberships(Object.fromEntries(membershipResults));
    } catch (reason) { onError(reason); }
  };
  useEffect(() => { void load(); }, [workspaces.length]);
  ...
  return <div className="modal-backdrop" onClick={close}><div className="modal platform-admin-modal" onClick={(event) => event.stopPropagation()}><div className="modal-head"><div><h3>平台用户</h3><p>查看每个账户对应的工作区权限，并管理平台角色和登录状态。</p></div><button aria-label="关闭" onClick={close}>×</button></div><div className="admin-user-list">{users.map((user) => { const assigned = workspaces.flatMap((workspace) => (workspaceMemberships[workspace.id] ?? []).filter((member) => member.user_id === user.id).map((member) => ({ workspace, role: member.role }))); const access = user.is_super_admin ? workspaces.map((workspace) => ({ workspace, role: "owner" as WorkspaceRole, inherited: !assigned.some((item) => item.workspace.id === workspace.id) })) : assigned.map((item) => ({ ...item, inherited: false })); return <div key={user.id}><div className="admin-user-identity"><strong>{user.name}{user.id === currentUser.id ? "（你）" : ""}</strong><small>{user.email}</small><div className="admin-access-label">空间权限</div><div className="member-workspace-access">{access.map(({ workspace, role: accessRole, inherited }) => <span key={workspace.id}><b>{workspace.name}</b><em>{accessRole}{inherited ? " · 平台继承" : ""}</em></span>)}{!access.length && <span className="empty"><b>暂无空间权限</b></span>}</div></div><span className={`account-status ${user.status}`}>{user.status === "active" ? "正常" : "已停用"}</span><label className="admin-role-switch"><input type="checkbox" checked={user.is_super_admin} disabled={user.id === currentUser.id} onChange={(event) => update(user, { is_super_admin: event.target.checked })} /><i aria-hidden="true" /><span>超级管理员</span></label><button className="account-state-action" disabled={user.id === currentUser.id} onClick={() => update(user, { status: user.status === "active" ? "disabled" : "active" })}>{user.status === "active" ? "停用" : "启用"}</button></div>; })}</div>{inviteLink && <div className="api-key-once"><strong>邀请链接仅显示一次，3 天内有效</strong><code>{inviteLink}</code><button onClick={() => navigator.clipboard.writeText(inviteLink)}>复制</button></div>}<form className="admin-user-create" onSubmit={create}><h4>邀请用户注册</h4><label><span>姓名</span><input name="name" placeholder="可选" /></label><label><span>受邀邮箱</span><input name="email" type="email" placeholder="name@example.com" required /></label><label className="invite-admin-option"><input name="is_super_admin" type="checkbox" /><i aria-hidden="true" /><span>设为平台超级管理员</span></label><button className="primary">生成邀请链接</button></form></div></div>;
}
