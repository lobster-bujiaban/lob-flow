...
export const api = {
  ...
  createDataset: (workspaceId: string, body: { name: string; description: string; icon: string; search_method?: string; vector_weight?: number }) => request<Dataset>(`/api/workspaces/${workspaceId}/datasets`, { method: "POST", body: JSON.stringify(body) }),
  ...
  addDocument: (datasetId: string, body: { name: string; content: string; separator?: string; max_chars?: number; overlap?: number; metadata?: Record<string, unknown> }) => request<DatasetDocument>(`/api/datasets/${datasetId}/documents`, { method: "POST", body: JSON.stringify(body) }),
  ...
  retrieveDataset: (datasetId: string, body: { query: string; top_k?: number; search_method?: string; vector_weight?: number; metadata_filter?: Array<{ key: string; operator: string; value?: unknown }> }) => request<{ query: string; results: RetrievalResult[]; duration_ms: number; search_method: string; embedding_model: string }>(`/api/datasets/${datasetId}/retrieve`, { method: "POST", body: JSON.stringify(body) }),
