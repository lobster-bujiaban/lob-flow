...
export function KnowledgeBase({ workspaceId, onError }: { workspaceId: string; onError: (reason: unknown) => void }) {
  ...
  async function addDocument(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      await api.addDocument(active.id, { name: String(data.get("name")), content: String(data.get("content")), separator: "\n\n", max_chars: 1200, overlap: 150, metadata });
      setModal(null); setDocuments(await api.listDocuments(active.id)); refresh();
    } catch (reason) { onError(reason); }
  }
  async function retrieve() {
    if (!active || !query.trim()) return;
    try {
      const parsed = metadataFilter.trim() ? JSON.parse(metadataFilter) as Record<string, unknown> : {};
      const filters = Object.entries(parsed).map(([key, value]) => ({ key, operator: "equals", value }));
      const response = await api.retrieveDataset(active.id, { query: query.trim(), top_k: 3, search_method: searchMethod, metadata_filter: filters });
      setResults(response.results); setDuration(response.duration_ms);
    } catch (reason) { onError(reason); }
  }
  ...
  return <section className="knowledge-detail">
    ...
    <div className="knowledge-columns"><div className="document-panel"><h3>文档 <span>{documents.length}</span></h3>{documents.map((doc) => <div className={`document-row ${documentId === doc.id ? "active" : ""}`} key={doc.id} onClick={() => setDocumentId(doc.id)}><div><strong>{doc.name}</strong><span>{doc.segment_count} 分段 · {doc.word_count} 字</span></div><button onClick={async (event) => { event.stopPropagation(); await api.enableDocument(doc.id, !doc.enabled); setDocuments(await api.listDocuments(active.id)); }}>{doc.enabled ? "已启用" : "已停用"}</button><button className="row-delete" onClick={async (event) => { event.stopPropagation(); if (confirm(`删除文档“${doc.name}”？`)) { await api.deleteDocument(doc.id); setDocuments(await api.listDocuments(active.id)); if (documentId === doc.id) setDocumentId(""); refresh(); } }}>×</button></div>)}{!documents.length && <div className="knowledge-empty">还没有文档，添加一段文本开始。</div>}</div>
      ...
      <aside className="retrieval-panel"><div className="inspector-title"><span>TEST</span><h3>召回测试</h3></div><p>输入问题，检查当前知识库能否返回正确片段。</p><label>检索方式</label><select value={searchMethod} onChange={(event) => setSearchMethod(event.target.value)}><option value="hybrid_search">混合检索</option><option value="vector_search">向量检索</option><option value="keyword_search">关键词检索</option></select><textarea rows={5} value={query} onChange={(event) => setQuery(event.target.value)} placeholder="输入检索问题" /><label>Metadata Filter（JSON，精确匹配）</label><textarea rows={3} value={metadataFilter} onChange={(event) => setMetadataFilter(event.target.value)} placeholder={'{"category":"manual","language":"zh"}'} /><button className="primary wide" onClick={retrieve} disabled={!query.trim()}>测试检索</button>{duration !== null && <small>{results.length} 条结果 · {duration} ms</small>}{results.map((result) => <article className="retrieval-result" key={result.segment_id}><header><strong>{result.document_name}</strong><span>{result.score.toFixed(4)}</span></header><small>向量 {result.vector_score.toFixed(4)} · 关键词 {result.keyword_score.toFixed(4)}</small><p>{result.content}</p></article>)}</aside>
    </div>
    {modal === "document" && <div className="modal-backdrop"><form className="modal document-modal" onSubmit={addDocument}><div className="modal-head"><div><h3>添加文本</h3><p>系统会按段落自动切分并建立向量索引</p></div><button type="button" onClick={() => setModal(null)}>×</button></div><label>文档名称</label><input name="name" required placeholder="例如：产品介绍" /><label>文本内容</label><textarea name="content" rows={12} required placeholder="粘贴需要检索的文本…" /><label>Metadata（JSON）</label><textarea name="metadata" rows={3} placeholder={'{"category":"manual","language":"zh"}'} /><div className="chunk-note">自动分段：最多 1200 字符 · 重叠 150 字符 · {active.embedding_model}</div><button className="primary wide">处理并添加</button></form></div>}
  </section>;
}
